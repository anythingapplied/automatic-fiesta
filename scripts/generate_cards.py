...
output_dir = '/home/dan/Projects/regicide/frontend/public/cards/'
...
def generate_card(rank, suit):
    is_royalty = rank in ['J', 'Q', 'K']
    border = '<rect x="4" y="4" width="192" height="272" rx="8" fill="none" stroke="#7c3aed" stroke-width="8" />' if is_royalty else ''
    
    return f"""<svg width="200" height="280" viewBox="0 0 200 280" xmlns="http://www.w3.org/2000/svg">
...
for rank in ranks:
    for suit in suits:
        file_name = f"{rank}{suit['code']}.svg"
        content = generate_card(rank, suit)
        with open(os.path.join(output_dir, file_name), 'w') as f:
            f.write(content)

# Generate Joker
with open(os.path.join(output_dir, 'Joker.svg'), 'w') as f:
    f.write(generate_joker())

print('Successfully generated 53 cards.')
